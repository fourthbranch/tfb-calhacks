...
def list_articles_display(request: Request):
   print("fsdffsfds")
   # Get user email from request headers
   user_email = request.headers.get("user_email")
   print("reached server side, user email is:", user_email)
   if not user_email:
       raise HTTPException(status_code=401, detail="Unauthorized: Missing user email in headers")
  ...
  # Filter articles into user_preferred and explore
   ...
   for article in articles_with_created_at:
       ...
       matches_topics = any(topic in preferred_topics for topic in article.get("relevant_topics"))
       matches_political_leaning = article_political_leaning == political_leaning
       matches_writing_style = article.get("preferred_writing_style") == preferred_writing_style
